...
let { title, gender, year } = game;  // uso desetructuring. Ya puedo acceder a las propiedades. 
...
const {name01, race02} = animalFunction();   // se puede usar desestructuring tambien para objetos {} y no solo para arrays []. Con esto puedes acceder a los valores de las propiedades del objeto. (Objeto tiene propiedades y estas tienen valores)  

console.log(name01); // te da el valor Bengal Tiger 
console.log(race02); // te da valor Tiger 
...
let { name, itv } = car;  // desestructuring para acceder a propiedaes del objeto car 
let [itv01, itv02, itv03] = car.itv; // desestructuring para acceder al array de la propiedad itv. Para conseguir esto car.itv (accedo a al objeto y luego a su propiedad)
...
let newToy = { ...toy }; // spread operator también puede crear copias de objetos 
...
const newColors = [...colors];  // creo copia del array 
newColors.splice(2, 1);  // uso splice para eliminar desde la posición 2, 1 elemento 
...
let newUsersMap = users.map((user => user.nameMap));   // con map recorro el array dentro de users y accedo a la propiedad .nameMap
...
const result02 = usersAnacleto.map(user => {        // map que recorre usersAnacleto y dentro tiene un if 
    if (user.name.charAt(0) == "A")                 // si el user.name (del recorrido que hace el map) empieza por A 
        return "Anacleto";                          // devuelve "Anacleto" 

    else
        return usersAnacleto.name;                  // si no devuelve el nombre normal 
});
...
const newCities = cities.map(city => {             // map para recorrer el array que está en cities 
	if (city.isVisited){                           // condicional si la city ha sido visitada. Es decir, si la propiedad isVisited tiene un valor de true  
		return city.name + " Visitado" ;           // return city.name + visitado. Ciudad más su nombre y le suma la palabra visitado 
	} else {
		return city.name;                          // si no devuelve el nombre normal 
	}
}); 
...
const newAgesFilter = ages.filter((age) => age > 18);   // si edad es mayor que 18 
...
const streamersFilter = streamers.filter(streamer => streamer.gameMorePlayed === "League of Legends");   // accedo a la propiedad de cada objeto en cada vuelta del filter y si es ==== a lo que busco filtro 
...
let newStreamers = streamers02.filter(streamer01 => streamer01.name.includes("u"));   // accedo al name de cada objeto y si contiene U lo filtro 
...
const moviesFind = movies.find(movie => movie.date === 2010);   // recorro movie con find hasta que encuentro ==== a date 2010 
console.log(moviesFind.title); // .tittle es para poner el valor del title y no el objeto en sí. 
...
const examResults = exams3.reduce(function(acc, alumnScore){
	...
	console.log(examResults / exams3.length); // para conseguir la media 
...
const result = videogames.filter(game => {
    const isRPG = game.genders.includes("RPG");  // primero se filtra para obtener los que dentro del objeto y su propiedad genders tengan RPG como valor 
...
.reduce((acumulador, current) => {
    const counter = acumulador + current.score;        // después se hace un reduce solo para sumar esos 
